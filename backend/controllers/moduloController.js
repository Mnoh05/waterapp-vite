...
const {createModulo, allModulos, moduloById} = require("../handlers/moduloHandler.js")
...
const createNewModulo = async(req, res) =>{
    ...
    try {
        ...
        const newModulo = await createModulo (nameModulo, latitud,longitud,chofer_id);
        return res.status(200).json(newModulo);

    } catch (error) {
        return res.status(404).json({message:"Error al crear nuevo modulo"});
    }
}
...
const findModuloByID = async(req, res) => {
    const { id} = req.body; 
        ...
        if(!id || isNaN(id)){//valida que el id que se ingrese sea un valor valido
...
module.exports = {createNewModulo, listAllModulos, findModuloByID}